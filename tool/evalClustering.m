function res = evalClustering(Y, predY)
% struct of clustering result
% [1] acc
% [2] nmi_max
% [3] nmi_sqrt
% [4] purity
...
[confus,accuracy,numcorrect,precision,recall,f_measure] = compute_accuracy_F (Y,res,1:nclass);
% NMI
[nmi_max, nmi_sqrt] = MutualInfo(Y,res);
res = struct('acc', accuracy, 'nmi_max', nmi_max, 'nmi_sqrt', nmi_sqrt', 'purity', purity, ...
    'precision', precision, 'recall', recall, 'f1', f_measure, ...
    'confus', confus);
end
...
function [nmi_max, nmi_sqrt] = MutualInfo(L1,L2)
%   mutual information
