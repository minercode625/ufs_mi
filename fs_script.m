...
for k = 1:length(list)
    ...
    fs_size = 5:5:max_col;
    res = struct('alg', {}, 'S', {}, 'S_std', {}, 'E', {}, 'E_std', {},  'R', {}, 'R_std', {}, 'F', {}, 'F_std', {});
    for alg_idx = 1:length(param_struct)
        ...
        f_std_table = zeros(length(param), length(fs_size));
        for param_idx = 1:length(param)
            ...
            parfor fs_idx = 1:length(fs_size)
                fs = fs_size(fs_idx);
                X_fs = X(:, fea(1:fs));
                [m_res, s_res] = kmeans_rng(X_fs, Y, rep_size, X);
                ...
                f_table(param_idx, fs_idx) = m_res(1,4);
                f_std_table(param_idx, fs_idx) = s_res(1,4);
            end
        end
        res(alg_idx).alg = alg;
